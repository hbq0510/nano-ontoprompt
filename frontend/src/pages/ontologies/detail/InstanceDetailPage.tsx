...
import { ArrowLeft, Pencil, Save, X, Plus, Code2, Zap, Trash2, Box, Play, RefreshCw } from 'lucide-react'
                ...
                {Object.entries(schema).map(([name, def]) => (
                  <div key={name}><label className="block text-[10px] text-gray-500 mb-0.5">{name}{def.unit ? ` (${def.unit})` : ''}</label>
                    {def.type === 'boolean' ? (
                      ...
                      <input type="number" value={editProps[name] || ''} onChange={e => setEditProps({ ...editProps, [name]: e.target.value })} className="w-full border rounded px-2 py-1 text-xs" />
                    ) : (
                      <input value={editProps[name] || ''} onChange={e => setEditProps({ ...editProps, [name]: e.target.value })} className="w-full border rounded px-2 py-1 text-xs" />
                    )}
                  </div>
                ))}
              </div>
        ...
        {editPropsOnly ? (
              ...
              {Object.entries(editProps).map(([name, v]) => (
                <div key={name} className="flex items-center gap-1">
                  <div className="flex-1">
                    <label className="block text-[10px] text-gray-500 mb-0.5">{name}{schema[name]?.unit ? ` (${schema[name].unit})` : ''}{!schema[name] ? <span className="text-amber-500 ml-1">(自定义)</span> : null}</label>
                    {schema[name]?.type === 'boolean' ? (
                      ...
                      <input type="number" value={v} onChange={e => setEditProps({ ...editProps, [name]: e.target.value })} className="w-full border rounded px-2 py-1 text-xs" />
                    ) : (
                      <input value={v} onChange={e => setEditProps({ ...editProps, [name]: e.target.value })} className="w-full border rounded px-2 py-1 text-xs" />
                    )}
                  </div>
          ...
          <div className="flex flex-wrap gap-2">
            {Object.entries(inst.properties || {}).map(([name, value]) => (
              <span key={name} className="inline-flex items-center gap-1 px-2.5 py-1 rounded text-xs font-mono bg-blue-50 text-blue-700">
                {name}={String(value)}{schema[name]?.unit ? ` ${schema[name].unit}` : ''}
              </span>
            ))}
          </div>
        )}
